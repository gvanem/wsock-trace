...
#if defined(TEST_WX_STKWALK)

class MyStackWalker : public wxStackWalker
{
  public:
    MyStackWalker() { m_numFrames = 0; }
  ...
  protected:
    virtual void OnStackFrame (const wxStackFrame& frame) wxOVERRIDE
    {
      ...
      printf ("[%02u] ", m_numFrames);

      const wxString name = frame.GetName();
      if ( !name.empty() )
      {
          printf ("%-40s", (const char*) name.c_str());
      }
      else
      {
          printf ("%p", frame.GetAddress());
      }

      if ( frame.HasSourceLocation() )
      {
          printf ("\t%s: %u", (const char*)frame.GetFileName(), frame.GetLine());
      }
