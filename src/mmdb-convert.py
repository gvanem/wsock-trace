...
#
# Changes by Gisle Vanem for Wsock_trace.
#
...
import os, sys, struct, bisect, socket, binascii, time, argparse
...
def parse_search_tree (s, record_size):
  ...
  try:
    ...
    raise NotImplementedError ("Unsupported record size in bits: %d" % record_size)
  while p < len(s):
...
def write_csv_file (f_out, metadata, the_tree, dump_item, fmt_item):
  """Write the entries in the_tree to out_file.
  """
  entries = []
  ...
  for entry in entries:
    if not unwritten:
      ...
      f_out.write (fmt_item(unwritten))
      unwritten = entry
  if unwritten:
    f_out.write (fmt_item(unwritten))

def usage():
  print ("Usage: %s [-46h] -i <in_file> -o <out_file>:\n"
         "  -i <in_file>:  The input MMDB file to convert      (required).\n"
         "  -o <out_file>: The output CSV file to generate     (required).\n"
         "  -4:            Write the output as AF_INET records (default).\n"
         "  -6:            Write the output as AF_INET6 records.\n"
         "  -h:            show this help." % who_am_I)
  sys.exit (0)
...
def main():
  ...
  parser.add_argument ("-4", dest="af_inet", action="store_true")
  parser.add_argument ("-6", dest="af_inet6", action="store_true")

  opt = parser.parse_args()
  if opt.show_help:
    usage()

  f_out = open (opt.out_file, 'w')
  f_in  = open (opt.in_file, 'rb')

  print ("Reading and parsing content")
  metadata, the_tree, _ = parse_mm_file (f_in.read())

  f_out.write ("#\n")
  f_out.write ("# Converted by '%s %s%s -i %s -o %s'\n" % (sys.executable, __file__,  [""," -6"][opt.af_inet6], opt.in_file, opt.out_file))
  f_out.write ("# at %s.\n" % time.ctime())
  f_out.write ("#\n")

  if opt.af_inet6:
    print ("Dumping AF_INET6 information to %s" % opt.out_file)
    write_csv_file (f_out, metadata, the_tree, dump_item_ipv6, fmt_item_ipv6)
  else:
    print ("Dumping AF_INET information to %s" % opt.out_file)
    write_csv_file (f_out, metadata, the_tree, dump_item_ipv4, fmt_item_ipv4)
...
if __name__ == '__main__':
  if sys.version[0] != '3':
    print ("This script needs Python3.x")
    sys.exit(1)
  main()
