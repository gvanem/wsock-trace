...
if %1. == --ip2loc_4. (
  grep.exe --only-matching "[0-9\.]*" ..\IP2Location-C-Library\test\country_test_ipv4_data.txt > %TEST_INPUT%
  rem
  rem Add some IPv4 addresses from SpamHaus' DROP.txt too:
  rem
  echo 36.37.48.34     # part of 36.37.48.0/20   ; SBL258006 >> %TEST_INPUT%
  echo 121.46.124.33   # part of 121.46.124.0/22 ; SBL287428 >> %TEST_INPUT%
  %~dp0geoip.exe -4 %2 %3 %4 @%TEST_INPUT%
  exit /b 0
)

if %1. == --ip2loc_6. (
  grep.exe --only-matching "[0-9a-f\:]*" ..\IP2Location-C-Library\test\country_test_ipv6_data.txt > %TEST_INPUT%
  rem
